---
title: "Análisis de la desigualdad en el mundo (coeficiente GINI)"
output:
  word_document:
...
En el gráfico se puede percibir que la desigualdad tiende a mantenerse constante en el periódo estudiado, destacando **Latinoamérica y el Caribe** como la región más desigual pero la que pareciera tener la mayor disminución. El máximo valor corresponde a `r datos_graf_1[datos_graf_1$gini == max(datos_graf_1$gini),]$region` el año `r datos_graf_1[datos_graf_1$gini == max(datos_graf_1$gini),]$anio` mientras que el menor valor corresponde a `r datos_graf_1[datos_graf_1$gini == min(datos_graf_1$gini),]$region` el año `r datos_graf_1[datos_graf_1$gini == min(datos_graf_1$gini),]$anio`.

Considerando que `r datos_graf_1[datos_graf_1$gini == max(datos_graf_1$gini),]$region` es la región con mayor desigualdad, haremos un *zoom* en los países que lo componen. El siguiente gráfico muestra el promedio del coeficiente GINI para cada uno de los países de la región en el periódo estudiado.
...
datos_graf_2 <- 
  ...
  filter(region == datos_graf_1[datos_graf_1$gini == max(datos_graf_1$gini),]$region, !is.na(gini)) %>%
  # Agrupar datos por país
  group_by(pais) %>% 
  # Para cada país calcular el promedio del coeficiente GINI
  summarise(gini = round(mean(gini, na.rm = TRUE), 1)) 
...
reg3 <- lm(gini ~ pib_percapita + gasto_educacion + desempleo + gasto_militar + `gasto_I&D` + grupo_ingresos + region + anio, data = datos) 
```

```{r, include = FALSE}
stargazer(reg1, reg2, reg3, type = "html", out = "Anexo I.doc")
```
